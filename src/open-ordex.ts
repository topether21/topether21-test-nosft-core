...
async function fetchBitcoinPrice({
  ...
  baseMempoolApiUrl,
  feeLevel,
}: fetchBitcoinPriceProps): Promise<{
  bitcoinPrice: number;
  ...
  const recommendedFeeRateResponse = await fetch(
...
async function getInscriptionDataById(
  ...
  try {
    ...
    if (numberMatch) {
      ...
      console.error(
        `Failed to find inscription number for ID ${inscriptionId}`
      );
    }
  } catch (error) {
    console.error(
      `Failed to parse inscription data for ID ${inscriptionId}: ${error}`
    );
  }

  if (
    verifyIsInscriptionNumber &&
    String(inscriptionNumber) !== verifyIsInscriptionNumber
  ) {
...
function satsToFormattedDollarString(
  sats: number,
  bitcoinPrice: number
): string {
...
class OpenOrdexFactory {
  ...
  async validateSellerPSBTAndExtractPrice(
  ...
  async getProcessedOrder(
    ...
    const inscriptionDataResponse = await fetch(
    ...
    const validatedPrice = await this.validateSellerPSBTAndExtractPrice(
      order.content,
      inscriptionRawData.output
    );
